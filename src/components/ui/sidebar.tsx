...
const SidebarContext = React.createContext<
  ...
  SidebarContextProps<any> | undefined
>(undefined);
    ...
    (
      ...
      setSidebars((prev) => {
        ...
        document.cookie = `${openState
          .map(
            (sidebarName) =>
              `${sidebarName}:state=${next[sidebarName].isOpen}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`,
          )
          .join("; ")}`;
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            "contents group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar  min-h-svh w-full",
            className,
          )}
          {...props}
        >
...
function Sidebar<T extends string>({
  ...
  return (
    <div
      className="~contents group peer text-sidebar-foreground hidden md:block"
    ...
    >
      ...
      >
        <div
          ...
          className="bg-sidebar group-data-[variant=floating]:border-sidebar-border contents ~flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm"
        >
          {children}
        </div>
      </div>
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "contents ~flex min-h-0 flex-1 flex-col gap-2 overflow-auto group-data-[collapsible=icon]:overflow-hidden",
        className,
      )}
      {...props}
    />
